...
import (
	...
	mockdb "github.com/luislard/simplebank/db/mock"
	...
	testCases := []struct {
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				store.EXPECT().CreatAccount(gomock.Any(), arg).
					Times(1)
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().CreatAccount(gomock.Any(), gomock.Any()).
					Times(0)
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusBadRequest, recorder.Code)
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
					CreatAccount(gomock.Any(), gomock.Any()).
					Times(1).
					Return(db.Account{}, sql.ErrConnDone)
